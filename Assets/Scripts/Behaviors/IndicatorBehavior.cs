    ...
    protected override void PlausibleUpdate()
    {
        ...
        if (On)
        {
            ...
            else
            {
                ...
                var targetDirection = mainCamera.ViewportToWorldPoint(new Vector3(.5f, .5f, screenPosition.z)) -
                                      Target.position;
                var angle = Vector2.SignedAngle(Vector2.down, new Vector2(targetDirection.x, targetDirection.z));

                transform.SetPositionAndRotation(indicatorWorldPosition,
                    Quaternion.Euler(90, 0, angle)); // (Vector3.up,angle); //Quaternion.Euler(0, 0, angle) ;
